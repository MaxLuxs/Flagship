    ...
    LaunchedEffect(Unit) {
        try {
            ...
            if (success) {
                ...
                // Use sync methods after bootstrap attempt
                val hasFlags = try {
                    manager.isEnabledSync("new_feature", default = false)
                } catch (e: Exception) {
                    false
                }
            ...
            try {
                val manager = Flagship.manager()
                // Use sync method to check cache
                val hasFlags = try {
                    manager.isEnabledSync("new_feature", default = false)
                } catch (e2: Exception) {
                    false
    ...
    LazyColumn(
        ...
        item {
            Row(
                ...
                OutlinedButton(
                    onClick = {
                        // refresh() is not suspend, but we can call it directly
                        // Using scope.launch is fine for consistency
                        scope.launch {
                            try {
