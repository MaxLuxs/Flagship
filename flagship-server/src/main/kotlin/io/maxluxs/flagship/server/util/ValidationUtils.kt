...
object ValidationUtils {
    ...
    fun validateEmailWithResult(email: String): ValidationResult {
        ...
        if (email.length < MIN_EMAIL_LENGTH || email.length > MAX_EMAIL_LENGTH) {
            return ValidationResult(false, "Email must be between $MIN_EMAIL_LENGTH and $MAX_EMAIL_LENGTH characters")
        }
    ...
    fun validatePassword(password: String): ValidationResult {
        ...
        if (!hasUpper || !hasLower || !hasDigit) {
            return ValidationResult(false, "Password must contain at least one uppercase letter, one lowercase letter, and one digit")
        }
        return ValidationResult(true)
    }
    ...
    fun validateSlugWithResult(slug: String): ValidationResult {
        ...
        if (!SLUG_REGEX.matches(slug)) {
            return ValidationResult(false, "Slug must contain only lowercase letters, numbers, and hyphens")
        }
    ...
    fun validateFlagKeyWithResult(key: String): ValidationResult {
        ...
        if (!FLAG_KEY_REGEX.matches(key)) {
            return ValidationResult(false, "Flag key must contain only letters, numbers, underscores, and hyphens")
        }
    ...
    fun validateRestFlagValue(flagValue: RestFlagValue): ValidationResult {
        ...
        if (!validTypes.contains(flagValue.type)) {
            return ValidationResult(false, "Invalid flag type: ${flagValue.type}. Must be one of: ${validTypes.joinToString()}")
        }
    ...
    fun validateRestExperiment(experiment: RestExperiment): ValidationResult {
        ...
        if (kotlin.math.abs(totalWeight - 1.0) > weightTolerance) {
            return ValidationResult(false, "Sum of variant weights must equal 1.0 (current: $totalWeight)")
        }
